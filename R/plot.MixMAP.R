...
max.pos<-NULL
for (i in 1:21){
max.pos[i]<-max(manhat.ord$BP[manhat.ord$CHR==i])}
...
manhat.ord$pos<-manhat.ord$BP+max.pos2[as.numeric(manhat.ord$CHR)]
manhat.ord$post.est[manhat.ord$post.est>0]<-0
manhat.ord$post.est[manhat.ord$post.est<0]=abs(manhat.ord$post.est[manhat.ord$post.est<0])
...
#plot the data
plot(manhat.ord$pos/1000000,manhat.ord$post.est,pch=20,col=manhat.ord$col,xlab="Chromosome",ylab="Absolute Value of Empirical Bayes Estimate",axes=F,main=title)
axis(2)
abline(h=0)
...
MixMAPandSNPGenes<-as.character(x@detected.genes[x@detected.genes$pval.min<(0.05/x@num.genes.detected[2]),1])
...
points(manhat.ord$pos[manhat.ord[,1]%in%MixMAPonlyGenes]/1000000,manhat.ord$post.est[manhat.ord[,1]%in%MixMAPonlyGenes],pch=15,col=col.detected[1], bg=col.detected[1],cex=2)

#Were any genes detected in both SNP and MixMAP?
if (any(x@detected.genes$pval.min<0.05/sum(x@output$SNP.COUNT))){
#Plot MixMAP and Single SNP detected genes
points(manhat.ord$pos[manhat.ord[,1]%in%MixMAPandSNPGenes]/1000000,manhat.ord$post.est[manhat.ord[,1]%in%MixMAPandSNPGenes],pch=19,col=col.detected[2], bg=col.detected[2])
}

}

#Display gene names of detected genes
if (display.text==TRUE & x@num.genes.detected[1]>0){
	
text(manhat.ord$pos[manhat.ord[,1]%in%MixMAPonlyGenes]/1000000,manhat.ord$post.est[manhat.ord[,1]%in%MixMAPonlyGenes],as.character(manhat.ord[manhat.ord[,1]%in%MixMAPonlyGenes,1]),col=col.text,offset=.6,pos=4, cex=.8)

if (any(x@detected.genes$pval.min<0.05/sum(x@output$SNP.COUNT))){
text(manhat.ord$pos[manhat.ord[,1]%in%MixMAPandSNPGenes]/1000000,manhat.ord$post.est[manhat.ord[,1]%in%MixMAPandSNPGenes],as.character(manhat.ord[manhat.ord[,1]%in%MixMAPandSNPGenes,1]),col=col.text,offset=.6,pos=4, cex=.8)}
}
}
